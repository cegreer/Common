...
namespace Vizistata.Data {
	...
	/// </summary>
	public sealed class DataRecord : DbDataRecord, ICustomTypeDescriptor {
	// Fields
		/// <summary>
	...
	// Constructors
		...
		internal DataRecord(ColumnInfo[] columnInfos, Object[] values)
			: base() {
			Debug.Assert(columnInfos != null);
			Debug.Assert(values != null);
			Debug.Assert(columnInfos.All(columnInfo => columnInfo != null));
			Debug.Assert(columnInfos.Where(columnInfo => columnInfos.Any(other => columnInfo != other && columnInfo.Ordinal == other.Ordinal)).Count() == 0);
			Debug.Assert(columnInfos.Where(columnInfo => columnInfos.Any(other => columnInfo != other && columnInfo.Name == other.Name)).Count() == 0);
			Debug.Assert(columnInfos.Length == values.Length);
	...
	// Properties
		...
		public override Int32 FieldCount {
			get { return this._columnInfos.Length; }
		}
	...
	// Methods
		...
		public override Int64 GetBytes(Int32 i, Int64 dataIndex, Byte[] buffer, Int32 bufferIndex, Int32 length) {
			Byte[] byteArray = (Byte[])this._values[i];
		...
		public override Int64 GetChars(Int32 i, Int64 dataIndex, Char[] buffer, Int32 bufferIndex, Int32 length) {
			String value = (String)this._values[i];
		...
		public override String GetDataTypeName(Int32 i) {
			return this._columnInfos[i].TypeName;
		}
		/// <summary>
		...
		public override Type GetFieldType(Int32 i) {
			return this._columnInfos[i].Type;
		}
		/// <summary>
		...
		public override String GetName(Int32 i) {
			return this._columnInfos[i].Name;
		}
		/// <summary>
		...
		public override Int32 GetOrdinal(String name) {
			...
			ColumnInfo foundColumnInfo = this._columnInfos
				.Where(columnInfo => String.Equals(columnInfo.Name, name, StringComparison.OrdinalIgnoreCase))
				.SingleOrDefault();
			if (foundColumnInfo == null) {
				throw new IndexOutOfRangeException("The column name is not in the list of columns.");
			}
			return foundColumnInfo.Ordinal;
		}
		/// <summary>
		...
		public override Int32 GetValues(Object[] values) {
			...
			Int32 itemsToCopyCount = Math.Min(values.Length, this._values.Length);
		...
		PropertyDescriptorCollection ICustomTypeDescriptor.GetProperties(Attribute[] attributes) {
			return new PropertyDescriptorCollection(this._columnInfos.Select(columnInfo => columnInfo.PropertyDescriptor).ToArray());
		}
